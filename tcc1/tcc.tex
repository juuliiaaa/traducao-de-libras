...
\begin{enumerate}
	...
	\item Segmentação e extração de \textit{landmarks}: A região de interesse é isolada, as mãos e o rosto são detectados, e o MediaPipe extrai as coordenadas tridimensionais ($x$, $y$, $z$) dos pontos articulares. Cada mão detectada contribui com um vetor de $21 \times 3 = 63$ valores, ou seja, 21 pontos-chave, cada um com 3 coordenadas; como uma pessoa pode sinalizar com uma ou duas mãos, esses vetores por mão são concatenados, totalizando até 126 valores quando ambas as mãos estão presentes. A esses, são concatenados os valores dos pontos faciais relevantes extraídos pelo \textit{MediaPipe Face Mesh}, utilizados para a classificação das Expressões Não Manuais (ENM). É esse vetor numérico de \textit{landmarks} -- e não a imagem bruta -- que serve de entrada para os modelos de reconhecimento das etapas seguintes, reduzindo o espaço de análise ao que é semanticamente relevante para o sinal.
	\item Roteamento por tipo de sinal: Com base na presença ou ausência de deslocamento significativo dos \textit{landmarks} ao longo de \textit{frames} consecutivos, o sistema direciona o vetor (ou a sequência de vetores) para um dos dois ramos de classificação a seguir, que são paralelos e independentes entre si.
	\item Ramo estático (CNN): Para sinais sem componente de movimento relevante, o vetor de \textit{landmarks} de um único \textit{frame} é tratado como entrada estruturada da CNN. As camadas de convolução aplicam filtros sobre esse vetor -- não sobre pixels de uma imagem -- para detectar combinações discriminativas entre as posições articulares, como o padrão de flexão dos dedos característico de cada configuração de mão. Em seguida, camadas de \textit{pooling} reduzem a dimensionalidade das representações obtidas, tornando-as mais compactas e menos suscetíveis ao \textit{overfitting}.
	\item Ramo dinâmico (LSTM): Para sinais com movimento, em vez de um único vetor, a entrada é uma sequência de vetores de \textit{landmarks} correspondente aos \textit{frames} do gesto. A LSTM processa essa sequência \textit{frame} a \textit{frame}, utilizando seus portões de entrada, esquecimento e saída para reter, ao longo do tempo, a informação de trajetória necessária para diferenciar sinais que dependem do movimento.
...
\begin{figure}[H]
    ...
    \resizebox{\textwidth}{!}{
        \begin{ganttchart}[
            ...
            \ganttgroup{Fase 1: Planejamento}{1}{3} \\
            \ganttbar[name=req]{Requisitos do sistema}{1}{2} \\
            \ganttbar[name=analise]{Análise do \textit{dataset}}{1}{2} \\
            \ganttbar[name=arq]{Projeto da arquitetura}{2}{3} \\
            \ganttmilestone[name=m1]{Entrega projeto}{3} \\
 
            \ganttgroup{Fase 2: Desenvolvimento}{4}{12} \\
            \ganttbar[name=visao]{Módulo Visão Computacional}{4}{6} \\
            \ganttbar[name=cnn]{Treinamento CNN}{5}{7} \\
            \ganttbar[name=lstm]{Implementação LSTM}{6}{8} \\
            \ganttbar[name=pln]{Desenvolvimento PLN + LLM}{6}{10} \\
            \ganttbar[name=integ]{Integração módulos}{9}{11} \\
            \ganttbar[name=interf]{Interface}{9}{12} \\
            \ganttmilestone[name=m2]{Protótipo}{12} \\
            ...
            \ganttbar{Redação final}{1}{16}
 
            \ganttlink{req}{arq}
            \ganttlink{analise}{arq}
            \ganttlink{visao}{cnn}
            \ganttlink{cnn}{lstm}
            \ganttlink{lstm}{integ}
            \ganttlink{pln}{integ}
            \ganttlink{integ}{interf}
        \end{ganttchart}
    }
...
O cronograma proposto está organizado em três fases. A Fase 1 (Planejamento) compreende a definição dos requisitos e a análise do \textit{dataset}, executadas em paralelo, convergindo para o projeto da arquitetura na semana 3. 

A Fase 2 (Desenvolvimento) concentra a implementação dos módulos do \textit{pipeline}: o módulo de visão computacional antecede o treinamento da CNN e a implementação da LSTM, enquanto o desenvolvimento do PLN e da camada de refinamento por LLM ocorre em paralelo, por não depender dessa etapa, encerrando-se com a integração dos módulos, o desenvolvimento da interface e a entrega do protótipo na semana 12. 
